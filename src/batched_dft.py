...
def batched_dft_r2r_AoS(
    ...
    # Create dst node 
    operator_name='batched_dft_r2r_AoS'
    state = sdfg.add_state(operator_name+str(int(10000*time.perf_counter_ns())), is_start_state=is_start_state)
...
def test_batched_DFT_r2r_N2(backend : str, Nr : int, Mr : int, dtype_input : str, aoptBool : bool):
    if dtype_input == 'complex128':
        dtype = np.float64
        dfttype = np.float64
        dtype_str = 'float64'
...
def test_batched_DFT_r2r_SoA(backend : str, Nr : int, Mr : int, dtype_input : str, aoptBool : bool):
    if dtype_input == 'complex128':
        dtype = np.float64
        dfttype = np.float64
        dtype_str = 'float64'
    ...
    sdfg_name = 'batched_DFTr2r_SoA'+'_'+dtype_input+'_'+aopt_str+'_'+str(Nr)
    sdfg = dc.SDFG(sdfg_name)
    N = dc.symbol('N', dtype=dc.int32)
    M = dc.symbol('M', dtype=dc.int32)
...
def test_batched_DFT_r2r_AoS(backend : str, Nr : int, Mr : int, dtype_input : str, aoptBool : bool):
    if dtype_input == 'complex128':
        dtype = np.float64
        dfttype = np.float64
        dtype_str = 'float64'
