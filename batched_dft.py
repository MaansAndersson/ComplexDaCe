...
import numpy as np
import os 
from operators_mem import transient_connector 
...
def test_batched_DFTc():
    sdfg = dc.SDFG('Test_batched_DFTc')
    N = dc.symbol('N', dtype=dc.int64)
    #M = dc.symbol('M', dtype=dc.int64)
    dtype_input = dc.complex128
    sdfg.add_array('x', [N], dtype=dtype_input)
    #sdfg.add_array('x2', [N], dtype=dc.complex128)

    sdfg.add_transient('t1', shape=[N], dtype=dtype_input)
    ...
    bstate = batched_dftc(sdfg, dtype_input, N, True, 'x', 'x')
...
def test_batched_DFTr2r():
    sdfg = dc.SDFG('Test_batched_DFTr2r')
    N = dc.symbol('N', dtype=dc.int64)
    #M = dc.symbol('M', dtype=dc.int64)

    sdfg.add_array('xr', [N], dtype=dc.float64)
    sdfg.add_array('xi', [N], dtype=dc.float64)

    # sdfg.add_transient('t1', shape=[N], dtype=dc.flaot)
    N = 256
    ...
    bstate = batched_dftr2r(sdfg, np.float64, N, True, 'xr', 'xi', 'xr', 'xi')
    #state1 = transient_connector(sdfg, False, 't1', 'x', None, None)
    #sdfg.add_edge(bstate, state1, dc.InterstateEdge())
...
def test_batched_DFT_r2r_N2():
    sdfg = dc.SDFG('Test_batched_DFTr2r_N2')
    N = dc.symbol('N', dtype=dc.int64)
    #M = dc.symbol('M', dtype=dc.int64)
...
def main():
    time1 = test_batched_DFTc()
    time2 = test_batched_DFTr2r()
    time3 = test_batched_DFT_r2r_N2()
